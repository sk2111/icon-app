...
//utils
import { ROUTE_MATCH_LIST, PLACEHOLDERMAP, KEYWORDS, SELECT_OPTIONS } from './home-header.util';
//route constants
...
const HomeHeader = ({ navMenuExpanded, setNavMenuExpanded, commonIconsKeywords,
    ...
    const renderHelper = () => {
        if (pathname === EDIT_ROUTE) return null;
        return (
            <React.Fragment>
                <SearchSelectDropdown className="ml-24" placeholder={placeHolderText} />
                {pathname !== FAVORITES_ROUTE ? <CustomSelect options={[]} /> : null}
            </React.Fragment>
        );
    };
    ...
    return (
        <div className={styles.headerContainer}>
            <div className={styles.headerLeftSec}>
                ...
                {isMatchFound ? renderHelper() : null}
            </div>
